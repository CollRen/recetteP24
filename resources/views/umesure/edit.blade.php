        ...
        <div class="md:w-1/3 pr-4 pl-4">
            <div class="relative flex flex-col min-w-0 rounded break-words border bg-white border-1 border-gray-300">
                ...
                <div class="flex-auto p-6">
                    <form action="{{ route('umesure.update',['umesure' => $umesure ]) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="mb-3">
                            <label for="umesure_en" class="form-label">umesure in English</label>
                            <input type="text" class="block appearance-none w-full py-1 px-2 mb-1 text-base leading-normal bg-white text-gray-800 border border-gray-200 rounded" id="umesure_en" name="umesure_en" value="{{ old('umesure_en', $umesure['en']) }}">
                        </div>
                        <div class="mb-3">
                            <label for="umesure_fr" class="form-label">umesure in French</label>
                            <input type="text" class="block appearance-none w-full py-1 px-2 mb-1 text-base leading-normal bg-white text-gray-800 border border-gray-200 rounded" id="umesure_fr" name="umesure_fr" value="{{ old('umesure_fr', $umesure['fr']) }}">
                        </div>
